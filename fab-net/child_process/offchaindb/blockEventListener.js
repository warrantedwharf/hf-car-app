...
const gatewayPath = path.resolve(__dirname, '..', '..', 'gateway');
const ccpPath = gatewayPath + '/networkConnection.yaml'
const ccpJSON = yaml.safeLoad(fs.readFileSync(gatewayPath + '/networkConnection.yaml', 'utf8'));
...
async function main() {
    try {
        ...
        const userExists = await wallet.exists('aman@gmail.com');
        if (!userExists) {
            console.log('An identity for the user "aman@gmail.com" does not exist in the wallet');
        ...
        await gateway.connect(ccpPath, { wallet, identity: 'aman@gmail.com', discovery: { enabled: true, asLocalhost: true } });
